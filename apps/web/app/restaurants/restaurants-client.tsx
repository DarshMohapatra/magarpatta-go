    ...
    <>
      ...
      <section className="pt-24 pb-8">
        <div className="mx-auto max-w-[1280px] px-4 sm:px-6 lg:px-10">
          ...
          <h1 className="mt-3 font-display text-[40px] sm:text-[56px] leading-[0.98] tracking-[-0.025em]">
            Vendors, <span className="italic text-[color:var(--color-primary)]">all in one list.</span>
          </h1>
          <p className="mt-3 text-[14.5px] text-[color:var(--color-muted)] max-w-2xl">
      ...
      <div className="sticky top-16 z-30 bg-[color:var(--color-background)]/85 backdrop-blur-md border-y border-[color:var(--color-foreground)]/8">
        <div className="mx-auto max-w-[1280px] px-4 sm:px-6 lg:px-10 py-3">
          {/* Search */}
          <div className="relative mb-3 max-w-md">
            <svg width="14" height="14" viewBox="0 0 14 14" fill="none" className="absolute left-3 top-1/2 -translate-y-1/2 text-[color:var(--color-muted)]/60">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2.5 rounded-full bg-[color:var(--color-surface)] border border-[color:var(--color-foreground)]/10 text-[13.5px] outline-none focus:border-[color:var(--color-primary)] placeholder:text-[color:var(--color-muted)]/50"
            />
          </div>
              ...
              <button
                ...
                className={cn(
                  ...
                  sortKey === s.key
                    ...
                    : 'bg-transparent hover:bg-[color:var(--color-foreground)]/5',
                )}
              >
                {s.label}
              </button>
      ...
      <section className="pb-24 pt-8">
        <div className="mx-auto max-w-[1280px] px-4 sm:px-6 lg:px-10">
          {filtered.length === 0 ? (
            <div className="rounded-2xl border border-[color:var(--color-foreground)]/10 bg-[color:var(--color-surface)] p-10 text-center">
              <p className="font-display text-[22px]">Nothing matches that.</p>
              <p className="mt-2 text-[13.5px] text-[color:var(--color-muted)]">Clear filters and try a different search.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-5">
              {filtered.map((v) => {
                ...
                <Link
                  ...
                  className="group rounded-2xl border border-[color:var(--color-foreground)]/10 bg-[color:var(--color-surface)] overflow-hidden hover:-translate-y-0.5 hover:shadow-[0_22px_50px_-24px_rgba(14,17,12,0.25)] transition-all"
                >
                  {/* Closed overlay */}
                  {!state.isOpen && (
                    <div className="absolute top-0 left-0 right-0 z-20 bg-[color:var(--color-foreground)]/90 text-[color:var(--color-background)] text-center text-[11px] uppercase tracking-[0.14em] py-1">
                  ...
                  <div className={cn("relative h-44 overflow-hidden flex", !state.isOpen && "opacity-60")}>
                    {v.previews.length > 0 ? (
                      v.previews.map((p, i) => (
                        <div key={p.id} className={cn('flex-1 relative', i > 0 && 'border-l border-[color:var(--color-surface)]')}
                    ...
                    {/* Top-right type pill */}
                    <div className="absolute top-3 right-3 rounded-full bg-[color:var(--color-surface)]/95 backdrop-blur px-2.5 py-1 text-[10.5px] uppercase tracking-[0.12em] text-[color:var(--color-muted)] border border-[color:var(--color-foreground)]/8">
                      {TYPE_META[v.vendorType]?.glyph ?? ''} {TYPE_META[v.vendorType]?.label ?? v.vendorType}
                    </div>

                    {/* Bottom-left rating chip */}
                    {v.rating && (
                      <div className="absolute bottom-3 left-3 rounded-full bg-[color:var(--color-foreground)] text-[color:var(--color-background)] px-2.5 py-1 text-[11.5px] font-semibold flex items-center gap-1">
                  ...
                  <div className="p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <h2 className="font-display text-[22px] leading-tight text-[color:var(--color-foreground)] truncate">
                          {v.name}
                        </h2>
                        <div className="mt-0.5 text-[12.5px] text-[color:var(--color-muted)] truncate">
                    ...
                    <div className={cn(
                      "mt-2 text-[11.5px]",
                      state.isOpen ? "text-[color:var(--color-primary)]" : "text-[color:var(--color-terracotta-dark)]",
                    )}>
                      {state.label}
                    </div>

                    <div className="mt-3 pt-3 border-t border-[color:var(--color-foreground)]/8 flex items-center justify-between text-[12px] text-[color:var(--color-muted)]">
    ...
    <button
      ...
      className={cn(
        ...
        active
          ...
          : 'bg-[color:var(--color-surface)] text-[color:var(--color-foreground)] border-[color:var(--color-foreground)]/10 hover:border-[color:var(--color-primary)]/40',
      )}
    >
