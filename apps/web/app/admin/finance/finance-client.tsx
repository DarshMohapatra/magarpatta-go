...
export function AdminFinanceClient() {
  ...
  return (
    <div>
      ...
      <div className="mt-6 flex gap-1 border-b border-[color:var(--color-ink)]/10">
...
function SettlementsTab({
  ...
  if (!data) return <div className="mt-6 text-[13px] text-[color:var(--color-ink-soft)]">Loading settlements…</div>;
  return (
    <>
      ...
      <div className="mt-4 flex justify-end">
        <button
          ...
          className="rounded-lg border border-[color:var(--color-ink)]/15 px-3 py-1.5 text-[12px] hover:border-[color:var(--color-forest)]/40"
        >
          Regenerate yesterday's settlements
        </button>
      </div>

      <section className="mt-6 rounded-2xl border border-[color:var(--color-ink)]/10 bg-[color:var(--color-paper)] overflow-hidden">
        ...
        {data.payable.length === 0 ? (
          <div className="p-6 text-center text-[13px] text-[color:var(--color-ink-soft)]">All settlements paid out.</div>
        ) : (
          <ul className="divide-y divide-[color:var(--color-ink)]/8">
            {data.payable.map((r) => (
              <li key={r.id} className="px-5 py-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-3 shrink-0">
                  <div className="text-right">
                    ...
                    <div className="text-[10px] uppercase tracking-[0.12em] text-[color:var(--color-ink-soft)]/65">net payable</div>
                  </div>
                  <button
                    onClick={() => onMarkPaid(r)}
                    disabled={busyId === r.id}
                    className="rounded-full bg-[color:var(--color-forest)] text-[color:var(--color-cream)] px-3.5 py-1.5 text-[12.5px] font-medium hover:bg-[color:var(--color-forest-dark)] disabled:opacity-50"
                  >
      ...
      <section className="mt-6 rounded-2xl border border-[color:var(--color-ink)]/10 bg-[color:var(--color-paper)] overflow-hidden">
        ...
        {data.paid.length === 0 ? (
          <div className="p-6 text-center text-[13px] text-[color:var(--color-ink-soft)]">No payouts settled yet.</div>
        ) : (
          <ul className="divide-y divide-[color:var(--color-ink)]/8">
            {data.paid.map((r) => (
  ...
  return (
    <button
      ...
      className={
        ...
        (active
          ? 'border-[color:var(--color-forest)] text-[color:var(--color-forest)]'
