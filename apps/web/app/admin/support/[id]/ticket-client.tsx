...
export function AdminTicketClient({ ticket }: { ticket: Ticket }) {
  return (
    <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-8">
      <div>
        <Link href="/admin/support" className="text-[12px] text-[color:var(--color-muted)] hover:text-[color:var(--color-foreground)]">← Queue</Link>
        ...
        <div className="mt-3 flex items-center gap-2 flex-wrap">
          ...
          <span className="text-[10px] uppercase tracking-[0.14em] px-2 py-0.5 rounded-full bg-[color:var(--color-foreground)]/8 text-[color:var(--color-muted)] font-medium">View only</span>
        </div>
        ...
        <div className="mt-8 bg-[color:var(--color-surface-2)] rounded-2xl p-5 text-center text-[12.5px] text-[color:var(--color-muted)]">
          Replies and status changes happen in the <b className="text-[color:var(--color-foreground)]">helpdesk portal</b> (<code className="text-[12px]">/helpdesk</code>). Admin oversight is read-only.
        </div>
      </div>
