    ...
    <div>
      ...
      <div className="mt-6 grid sm:grid-cols-[1fr_1.6fr] gap-5">
        <div className="rounded-2xl border border-[color:var(--color-primary)]/25 bg-gradient-to-br from-[color:var(--color-primary)]/8 to-[color:var(--color-moss)]/4 p-6">
          <div className="text-[10.5px] uppercase tracking-[0.16em] text-[color:var(--color-muted)]/70">Avg delivery rating</div>
          ...
          <div className="mt-1 text-[12.5px] text-[color:var(--color-muted)]/80">
            Across {data.stats.count} rating{data.stats.count === 1 ? '' : 's'}
          </div>
        </div>

        <div className="rounded-2xl border border-[color:var(--color-foreground)]/10 bg-[color:var(--color-surface)] p-5 space-y-1.5">
          {[...data.stats.distribution].reverse().map((d) => (
            <div key={d.star} className="flex items-center gap-2 text-[12px]">
              <span className="w-12 text-[color:var(--color-muted)]">{d.star} ★</span>
              <div className="flex-1 h-1.5 rounded-full bg-[color:var(--color-foreground)]/8 overflow-hidden">
      ...
      <section className="mt-8 rounded-2xl border border-[color:var(--color-foreground)]/10 bg-[color:var(--color-surface)] overflow-hidden">
        ...
        {data.feedback.length === 0 ? (
          <div className="p-8 text-center text-[13px] text-[color:var(--color-muted)]/75">
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 text-[11px] uppercase tracking-[0.12em]">
                      ...
                      <span className="text-[color:var(--color-muted)]/60">· #{f.order.id.slice(-6)}</span>
                    </div>
                    <div className="mt-1 text-[12.5px] text-[color:var(--color-muted)]/80 truncate">
                      {f.order.vendorName ?? 'Vendor'} → {f.order.building} · {f.order.society}
                    </div>
                    {f.deliveryComment && (
                      <p className="mt-2 text-[13px] leading-[1.55] text-[color:var(--color-foreground)]">&ldquo;{f.deliveryComment}&rdquo;</p>
                    )}
                  </div>
                  <div className="text-right shrink-0 text-[11px] text-[color:var(--color-muted)]/60">
...
function Stars({ rating }: { rating: number }) {
  return (
    <span className="text-[color:var(--color-saffron)]">
      {[1, 2, 3, 4, 5].map((n) => (
        <span key={n} className={n <= rating ? '' : 'text-[color:var(--color-foreground)]/15'}>★</span>
      ))}
    </span>
  );
}
