    ...
    <CuratorShell name={c.name}>
      ...
      <ul className="mt-7 divide-y divide-[color:var(--color-foreground)]/8 rounded-2xl border border-[color:var(--color-foreground)]/10 bg-[color:var(--color-surface)] overflow-hidden">
        ...
        {jobs.map((j) => {
          ...
          return (
            <li key={j.id}>
              <Link href={`/curator/${j.id}`} className="block px-5 py-4 hover:bg-[color:var(--color-background)]/40">
                <div className="flex items-center justify-between gap-3 flex-wrap">
                  <div className="min-w-0">
                    <div className="font-display text-[16px]">{j.vendor.name}</div>
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    ...
                    <span className="text-[11px] text-[color:var(--color-muted)]/65">
