...
export default async function SuperAdminSiteDetail({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <SuperShell phone={session.phone}>
      <Link href="/super-admin" className="text-[12.5px] text-[color:var(--color-muted)] hover:text-[color:var(--color-primary)]">
        ← Overview
      </Link>
      ...
      {!snapshot.ok ? (
        <div className="mt-6 rounded-2xl border border-[color:var(--color-terracotta)]/30 bg-[color:var(--color-terracotta)]/5 p-6">
          ...
          <h1 className="mt-2 font-display text-[32px]">Snapshot unavailable</h1>
          <p className="mt-3 text-[13px] text-[color:var(--color-terracotta-dark)]">{snapshot.error}</p>
          <p className="mt-4 text-[12.5px] text-[color:var(--color-muted)]">URL: {snapshot.url}</p>
        </div>
      ) : (
        <>
          <div className="mt-6">
            <div className="text-[10.5px] uppercase tracking-[0.16em] text-[color:var(--color-saffron)]">{snapshot.site?.platformName}</div>
            <h1 className="mt-2 font-display text-[36px] sm:text-[44px] leading-[1.02] tracking-[-0.02em]">{snapshot.site?.siteName}</h1>
            <p className="mt-2 text-[12.5px] text-[color:var(--color-muted)]">
              <a href={snapshot.url} target="_blank" rel="noreferrer" className="hover:text-[color:var(--color-primary)] underline underline-offset-2">{snapshot.url}</a>
              {' · '}
              <a href={`${snapshot.url}/admin`} target="_blank" rel="noreferrer" className="hover:text-[color:var(--color-primary)] underline underline-offset-2">Open this site&apos;s admin →</a>
            </p>
          </div>
          ...
          <div className="mt-10">
            <h2 className="font-display text-[22px] mb-3">Recent activity</h2>
            {(snapshot.recentActivity?.length ?? 0) === 0 ? (
              <p className="text-[13px] text-[color:var(--color-muted)]/70">No activity yet on this site.</p>
            ) : (
              <ul className="rounded-2xl border border-[color:var(--color-foreground)]/10 bg-[color:var(--color-surface)] divide-y divide-[color:var(--color-foreground)]/8 overflow-hidden">
                {snapshot.recentActivity!.map((a) => (
                  <li key={a.id} className="px-5 py-3 flex items-start justify-between gap-4">
                    <div className="min-w-0">
                      ...
                      <div className="mt-0.5 text-[13.5px] text-[color:var(--color-foreground)]">{a.summary}</div>
                    </div>
                    <div className="shrink-0 text-[11.5px] text-[color:var(--color-muted)]/65 whitespace-nowrap">
          ...
          <p className="mt-6 text-[10.5px] uppercase tracking-[0.14em] text-[color:var(--color-muted)]/55">
