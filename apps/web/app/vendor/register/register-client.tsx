...
const EMPTY: Form = {
  shopName: '', vendorType: 'restaurant', ownerName: '', ownerPhone: '', ownerEmail: '', password: '',
...
export function VendorRegisterClient() {
  ...
  const step1Valid = form.shopName && form.ownerName && form.ownerPhone.length === 10 && form.password.length >= 6;
  const step2Valid = true; // KYC optional at registration; flagged in admin review
      ...
      <div className="mx-auto max-w-[640px]">
          ...
          {[1, 2, 3].map((n) => (
            <li key={n} className="flex-1 flex items-center gap-2">
              ...
              <span className="text-[11.5px] uppercase tracking-[0.12em] text-[color:var(--color-ink-soft)]/75">
                {n === 1 ? 'Shop' : n === 2 ? 'KYC' : 'Payouts'}
              </span>
              {n < 3 && <div className="h-px flex-1 bg-[color:var(--color-ink)]/12" />}
            </li>
          ))}
        </ol>
        ...
        <div className="mt-8 rounded-3xl border border-[color:var(--color-ink)]/10 bg-[color:var(--color-paper)] p-6 sm:p-8 space-y-5">
          {step === 1 && (
            <>
              ...
              <div className="pt-3 border-t border-[color:var(--color-ink)]/8 space-y-5">
                ...
                <Row label="Password (6+ chars — you'll use this to sign in)">
                  <input type="password" value={form.password} onChange={(e) => set('password', e.target.value)} className={inp} />
                </Row>
              </div>
            </>
          )}
          ...
          {step === 3 && (
            <>
              <p className="text-[12.5px] text-[color:var(--color-ink-soft)]">
                Daily payouts land here. Commission ({<span className="font-medium text-[color:var(--color-ink)]">15%</span>}) is deducted automatically.
              </p>
