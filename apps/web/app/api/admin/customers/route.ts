...
export async function GET() {
  ...
  const users = await prisma.user.findMany({
    ...
    include: {
      ...
      orders: {
        select: { totalInr: true, status: true, placedAt: true, paymentMethod: true },
      },
    },
  });
  ...
  const rows = users.map((u) => {
    ...
    const prepaidDelivered = u.orders.filter((o) => o.status === 'DELIVERED' && o.paymentMethod !== 'COD').length;
    return {
      id: u.id,
      phone: u.phone,
      name: u.name,
      ...
      prepaidDelivered,
      codApprovedByAdmin: u.codApprovedByAdmin,
    };
  });
