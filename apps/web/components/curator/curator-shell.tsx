...
const NAV = [
  { href: '/curator', label: 'Queue' },
  { href: '/curator/history', label: 'History' },
];

export function CuratorShell({ name, children }: { name: string; children: React.ReactNode }) {
  ...
  async function signOut() {
    setSigningOut(true);
    try { await fetch('/api/curator/session', { method: 'DELETE' }); } catch {}
    startTransition(() => { router.push('/curator/signin'); router.refresh(); });
  }
        ...
        <div className="mx-auto max-w-[1280px] px-4 sm:px-6 py-3.5 flex items-center justify-between gap-3">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <span className="text-[14px] tracking-tight font-medium whitespace-nowrap">
              ...
              <span className="ml-1.5 text-[10.5px] uppercase tracking-[0.16em] text-[color:var(--color-saffron)]">Curator</span>
            </span>
