...
const toneClasses: Record<string, { bg: string; text: string; border: string }> = {
  forest: {
    bg: 'bg-[color:var(--color-primary)]',
    text: 'text-[color:var(--color-background)]',
...
export function Categories() {
  return (
    <section id="menu" className="py-24 lg:py-32 bg-[color:var(--color-surface-2)]/50 border-y border-[color:var(--color-foreground)]/8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-14">
          <div className="max-w-2xl">
            ...
            <h2 className="mt-4 font-display text-[44px] lg:text-[56px] leading-[0.98] tracking-[-0.02em]">
              Everything {siteConfig.wordmarkRoot} <span className="italic text-[color:var(--color-primary)]">already has.</span>
            </h2>
          </div>
          <p className="max-w-md text-[15px] leading-[1.6] text-[color:var(--color-muted)]">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-5">
          {CATS.map((c) => {
            ...
            return (
              <article
                key={c.title}
                className={`group relative overflow-hidden rounded-3xl border border-[color:var(--color-foreground)]/10 bg-[color:var(--color-surface)] p-7 lg:p-8 transition-all duration-500 hover:-translate-y-1 hover:shadow-[0_24px_60px_-24px_rgba(15,15,14,0.25)] ${
              ...
              >
                ...
                <h3 className="mt-6 font-display text-[30px] leading-tight text-[color:var(--color-foreground)]">
                  {c.title}
                </h3>
                <p className="mt-3 text-[14.5px] leading-[1.6] text-[color:var(--color-muted)]">
                  {c.desc}
                </p>
                ...
                <div className="mt-6 flex items-center justify-between">
                  ...
                  <Link href="/menu" className="inline-flex items-center gap-1 text-[13px] font-medium text-[color:var(--color-primary)] group-hover:translate-x-1 transition-transform">
                    Browse
