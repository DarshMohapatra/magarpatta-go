...
const NAV = [
  ...
  { href: '/admin/competitor-prices', label: 'Competitor prices' },
...
export function AdminShell({ name, role, children }: { name: string; role: string; children: React.ReactNode }) {
  ...
  async function signOut() {
    ...
    startTransition(() => { router.push('/admin/signin'); router.refresh(); });
  }
      ...
      <header className="border-b border-[color:var(--color-ink)]/8 bg-[color:var(--color-paper)] sticky top-0 z-20">
        <div className="mx-auto max-w-[1280px] px-4 sm:px-6 py-3.5 flex items-center justify-between gap-3">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <span className="ml-2 text-[10px] uppercase tracking-[0.14em] rounded-full px-2 py-0.5 bg-[color:var(--color-forest)]/12 text-[color:var(--color-forest)]">
